...
const COLUMNS: { key: string; label: string; color: string }[] = [
  { key: "new", label: "New Candidates", color: "border-t-primary-500" },
  { key: "shortlisted", label: "Shortlisted", color: "border-t-emerald-500" },
  { key: "interview", label: "Interview", color: "border-t-primary-400" },
  { key: "client_review", label: "Client Review", color: "border-t-secondary-500" },
              ...
              <div className="space-y-2 px-3 pb-3">
                {cards.map((card) => (
                  <article
                    ...
                    className="rounded-xl border border-[#E5E7EB] bg-white p-3 shadow-sm transition hover:border-primary-200 hover:shadow-md"
                  >
                    <div className="flex items-start gap-2.5">
                      <ClientAvatar name={card.name} size="sm" className="!bg-primary-100 !text-primary-700" />
                      ...
                      <Link
                        href={`/candidates/${card.candidate_id}`}
                        className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-[#9CA3AF] hover:bg-primary-50 hover:text-primary"
                      >
